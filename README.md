...
- `.gitignore` ja ignora dados, checkpoints, artefatos de EDA e submissoes para evitar vazamento de dados brutos e pesos.
- Remova arquivos sensiveis de `data/`, `outputs/` e `checkpoints/` antes do commit, ou mova-os para outro local.
- Inclua um LICENSE se quiser abrir o codigo; MIT eh uma opcao comum.

## Status atual
- `outputs/submission_transformer.csv` + `.meta.json`: exemplo gerado com as configuracoes default.
- `checkpoints/epoch*.pt`: pesos de treino (ignorados no git).
